...
- **Infraestructura y Pentesting de Red**
    ...
    - [**NetExec**](https://www.netexec.wiki/): (también conocido como nxc) Es una herramienta de explotación de servicios de red que ayuda a automatizar la evaluación de la seguridad de redes grandes.
    - [**Impacket**](https://github.com/fortra/impacket): Es un módulo de Python 3 para crear y analizar fácilmente protocolos de red. Se trata de una colección de clases de Python 3 centrada en proporcionar acceso a paquetes de red. Impacket permite a los desarrolladores de Python 3 crear y decodificar paquetes de red de forma sencilla y consistente. Incluye soporte para protocolos de bajo nivel como IP, UDP y TCP, así como para protocolos de alto nivel como NMB y SMB.
    - [**Rpcclient**](https://linux.die.net/man/1/rpcclient): Es una utilidad que forma parte del conjunto de herramientas Samba. Se utiliza para interactuar con el protocolo Remote Procedure Call (RPC) de Microsoft, que se utiliza para la comunicación entre los sistemas basados en Windows y otros dispositivos. Se utiliza principalmente para fines de depuración y pruebas, y se puede utilizar para consultar y manipular sistema remotos.
    - [**Httpbin**](https://httpbin.org/ip): En esta página podrás conocer cuál es la IP Pública de tu conexión a Internet.
    - [**BloodHound**](https://github.com/specterops/bloodhound): Herramienta fundamental para visualizar relaciones en Active Directory e identificar caminos de ataque (Attack Paths) hacia el Domain Admin.
...
- **Auditoría y Seguridad Web**
    ...
    - [**JWT Depurador**](https://www.jwt.io/): Decodifica, verifica y genera tokens web JSON.
    - [**JSNice**](http://jsnice.org/): Es una herramienta de ingeniería inversa esencial para analizar aplicaciones web y detectar amenazas ocultas. Su función principal es transformar código JavaScript "ilegible" en algo comprensible para un auditor.
    - [**Template Injection Table**](https://cheatsheet.hackmanit.de/template-injection-table/): Es una herramienta de referencia avanzada para pentesters creada por la firma de ciberseguridad Hackmanit. Su objetivo es simplificar drásticamente las fases de detección e identificación en auditorías de Inyección de Plantillas del Lado del Servidor (SSTI).
...
- **Escalada de Privilegios y Post-Explotación**
    ...
    - [**LinPEAS**](https://github.com/peass-ng/PEASS-ng/tree/master/linPEAS): Script en bash que localiza posibles vectores de elevación de privilegios en Linux.
    - [**WinPEAS**](https://github.com/peass-ng/PEASS-ng/tree/master/winPEAS): Herramienta que localiza posibles vectores de elevación de privilegios en Windows.
    - [**GTFOBins**](https://gtfobins.github.io/): Pagina muy recomendable de binarios de Unix que pueden usarse para eludir las restricciones de seguridad locales en sistemas mal configurados; También existe la posibilidad de usarlo a través de la terminal con la ayuda de la herramienta escrita en python [GTFOBins-cli](https://github.com/7Rocky/gtfobins-cli) o bien usar la herramienta escrita en shell [Searchbins](https://github.com/r1vs3c/searchbins).
...
- **Acceso Remoto y Shells**
    ...
    - [**Penelope**](https://github.com/brightio/penelope): Es un poderoso controlador de shell creado como un reemplazo moderno de netcat para la explotación de RCE, con el objetivo de simplificar, acelerar y optimizar los flujos de trabajo posteriores a la explotación.
    - [**Evil-winrm**](https://github.com/Hackplayers/evil-winrm): Este programa se puede usar en cualquier servidor Microsoft Windows con esta función habilitada (normalmente en el puerto 5985), solo si se tienen las credenciales y los permisos necesarios. Por lo tanto, podría utilizarse en la fase de hacking/pentesting posterior a la explotación. El objetivo de este programa es proporcionar funciones prácticas y fáciles de usar para el hacking. Los administradores de sistemas también pueden usarlo con fines legítimos, pero la mayoría de sus funciones se centran en el hacking/pentesting.

- **Reconocimiento y OSINT (Inteligencia de Fuentes Abiertas)**

    - [**Whatsystem**](https://github.com/peseta05/OS-Fingerprinter-Bash/tree/main): Es una herramienta de reconocimiento pasivo creada por mí y escrita en Bash que sirve para identificar el Sistema Operativo de un host remoto mediante el análisis de paquetes ICMP.
    ...
    - [**The Wayback Machine**](http://web.archive.org/): Es uno de los lugares más visitados para recuperar información de sitios web que han desaparecido. La web comenzo a realizar capturas de todos los sitios web de internet en 1996 y actualmente dispone de más de 700.000 millones de paginas web indexadas. Además, realiza una labor bibliotecaria, recuperando software, juegos e imágenes de otras épocas, que de otro modo desaparecerían. Incluso muchos juegos de videoconsolas antiguas se pueden jugar en línea en su web.
    - [**PhoneInfoga**](https://github.com/sundowndev/phoneinfoga): Es una herramienta de Inteligencia de Fuentes Abiertas (OSINT) especializada en el escaneo y recopilación de información sobre números de teléfono internacionales. Su objetivo principal es unificar búsquedas automáticas en registros públicos, motores de búsqueda y plataformas en línea para ayudar a investigadores de ciberseguridad, analistas y profesionales de IT a identificar la procedencia y la huella digital de un número telefónico.
    - [**Webmii**](https://webmii.com/?language=es): Busca información pública disponible en la web.
    ...
    - [**MACLookup**](https://maclookup.app/): Una utilidad imprescindible para fases de descubrimiento y auditoría de redes inalámbricas o cableadas. Permite introducir direcciones MAC completas o parciales para descubrir qué dispositivos específicos (móviles, routers, IoT) están conectados al entorno objetivo, facilitando la detección de intrusos o hardware desconocido.
    - [**Username Anarchy**](https://github.com/urbanadventurer/username-anarchy): Es una herramienta de código abierto diseñada para generar listas masivas de posibles nombres de usuario a partir del nombre y apellido de una persona.

- **Explotación y Cracking**
...
- **Hardware Hacking e IoT**
    ...
    - [**USB Rubber Ducky™**](https://docs.hak5.org/hak5-usb-rubber-ducky/usb-rubber-ducky-by-hak5/): Documentación oficial de este dispositivo de la marca I (2011) y II (2022), así como de su lenguaje de programación DuckyScript™ versión 1.0 (2011) y 3.0 (2022).
...
- **Ingeniería Inversa y Análisis de Malware**
    ...
    - [**Immunity Debugger**](https://github.com/kbandla/immunitydebugger): Es una potente herramienta de análisis dinámico de software orientada específicamente al desarrollo de exploits, análisis de malware e ingeniería inversa de binarios de Windows de 32 bits (x86). Basado originalmente en la interfaz clásica de [OllyDbg](https://sumble.com/tech/immunity-debugger), su principal ventaja competitiva radica en su robusta e integrada API de Python. Esta característica permite a los analistas automatizar tareas complejas mediante scripts, destacando el uso de plugins indispensables en ciberseguridad como [Mona.py](https://github.com/corelan/mona).
    - [**GDB con GEF (GDB Enhanced Features)**](https://github.com/hugsy/gef): Es la combinación de un depurador clásico de Linux con una extensión moderna diseñada específicamente para hacking ético, ingeniería inversa y desarrollo de exploits. Mientras que GDB estándar se maneja a ciegas mediante comandos de texto, GEF añade una potente interfaz visual automática dentro de la terminal. Cada vez que el programa se detiene, GEF divide la pantalla en paneles para mostrarte en tiempo real los registros de la CPU, el código ensamblador, la pila (stack) y las trazas de memoria.
    ...
    - [**Strings**](https://linux.die.net/man/1/strings): Es una herramienta que se utiliza para extraer cadenas de texto legibles desde archivos binarios.
    - [**Frida**](https://github.com/frida/frida): Es un popular framework de instrumentación binaria dinámica de código abierto utilizado por ingenieros inversos, analistas de malware y expertos en ciberseguridad para inspeccionar y modificar aplicaciones en tiempo real.
    - [**Ghidra**](https://github.com/NationalSecurityAgency/ghidra): Es una herramienta de ingeniería inversa gratuita y de código abierto desarrollada por la Agencia de Seguridad Nacional (NSA) de los Estados Unidos.
    - [**BinaryNinja**](https://binary.ninja/): Es una plataforma comercial de ingeniería inversa desarrollada por Vector 35, diseñada como una alternativa moderna, rápida y centrada en la automatización frente a IDA Pro y Ghidra. Lanzada por hackers y para hackers, destaca por su excelente interfaz de usuario y su potente motor de análisis interno.
    - [**ANY.RUN**](https://any.run/): Es un sandbox interactivo de malware basado en la nube diseñado para que profesionales de la ciberseguridad analicen archivos y enlaces sospechosos en un entorno seguro.
...
- **Otros Recursos**
    ...
    - [**F-Droid**](https://f-droid.org/es/): Es una tienda de aplicaciones alternativa para Android que distribuye exclusivamente software libre y de código abierto (FOSS). A diferencia de la Google Play Store, prioriza la privacidad del usuario, no requiere registrar una cuenta y permite descargar distintas versiones de una misma aplicación.
